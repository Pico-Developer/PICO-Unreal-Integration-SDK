...
{
	CurrentTrackingMode = EPXRBodyTrackingMode::PXR_BTM_NONE;
	TrackingStateCode = EPXRTrackingStateCode::PXR_MT_FAILURE;
}

FPXRBodyTrackingStartInfo::FPXRBodyTrackingStartInfo()
{
	StartMode=EPXRBodyTrackingMode::PXR_BTM_NONE;
}

FPXRBodyTrackingDataGetInfo::FPXRBodyTrackingDataGetInfo()
{
	DataFlags.SetNum(static_cast<int32>(EPXRBodyTrackingGetDataFlags::PXR_BODY_COUNT));
}

FPXRBodyTrackingData::FPXRBodyTrackingData()
{
	RoleDatas.SetNum(static_cast<int32>(EPxrBodyTrackerRole::Pxr_ROLE_COUNT));
}
