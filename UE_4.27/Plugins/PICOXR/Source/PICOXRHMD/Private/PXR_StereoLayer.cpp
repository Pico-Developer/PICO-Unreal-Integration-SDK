...
void GetTrackingSpaceDeltaPose(const FPXRGameFrame* Frame, FTransform& TrackingSpaceDeltaPose)
{
	...
	// Define: PXRPlugin-Device-FPose: same space with the PXRPlugin returned Pose
	// Define: UE4-Device-FPose:  the space UE4 seeing as device pose space. GetCurrentPose() returned result in this space
	FTransform SettingBasePose = FTransform(Frame->BaseOrientation, Frame->BaseOffset);
	PXR_LOGV(PxrUnreal, "AppSpaceWarp GetTrackingSpaceDeltaPose SettingBasePose Orientation:X:%f,Y:%f,Z:%f,W:%f"
		, Frame->BaseOrientation.X
		, Frame->BaseOrientation.Y
		, Frame->BaseOrientation.Z
		, Frame->BaseOrientation.W);

	// According context of GetCurrentPose, all PXRPlugin-Device-FPose will be inversed by Settings->BaseOrientation & Settings->BaseOffset
	// before exposing to UE4 as UE4-Device-FPose.  which means  UE4-Device-FPose = PXRPlugin-Device-FPose * FPose(BaseOrientation, BaseOffset ).Inverse()
	// FPose(BaseOrientation, BaseOffset ) is the UE4 device reference frame defined in PXRPlugin Device Space ( In FPose, notPxrPose ).
	// PXRPlugin-Device-FPose_To_UE4-Device-FPose = FPose(BaseOrientation, BaseOffset ).Inverse()
	// UE4-Device-FPose_To_PXRPlugin-Device-FPose = FPose(BaseOrientation, BaseOffset )

	// Eventually we want a transform in PXRPlugin space (both the input and output data are in PXRPlugin Device Space)
	TrackingSpaceDeltaPose = SettingBasePose.Inverse() * TrackingSpaceDeltaPose;
...
const void FPICOXRStereoLayer::SubmitLayer_RHIThread(const FGameSettings* Settings, const FPXRGameFrame* Frame)
{
	...
	if (ID == 0)
	{
		...
		if (CVarPICOEnableSpaceWarpInternal1->GetValueOnAnyThread())
		{
			...
			GetTrackingSpaceDeltaPose(Frame, TrackingSpaceDeltaPose);
