...
bool FPXR_DynamicResolutionState::IsSupported() const
{
	...
	PXR_LOGE(PxrUnreal, "Dynamic resolution for PICO is only supported on PICO Custom Engine.");
	return false;
#endif
...
void FPXR_DynamicResolutionState::SetupMainViewFamily(class FSceneViewFamily& ViewFamily)
{
	...
	{
		PXR_LOGE(PxrUnreal, "Cannot display screen resolution. Dynamic resolution not enabled.");
		return;
	}
...
#ifndef PICO_CUSTOM_ENGINE
	PXR_LOGE(PxrUnreal, "Cannot enable dynamic resolution. Must run on PICO Custom Engine.");
	return;
#endif
