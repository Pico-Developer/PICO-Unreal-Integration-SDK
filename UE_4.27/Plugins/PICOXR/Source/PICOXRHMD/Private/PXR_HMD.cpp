...
static TAutoConsoleVariable<bool> CVarPICOEnableEyeTrackingFoveationRendering(
	TEXT("r.Mobile.PICO.EnableEyeTrackingFoveationRendering"),
	true,
	TEXT("Whether to use eye tracking foveation rendering"),
	ECVF_Default);
#endif
...
#ifdef PICO_CUSTOM_ENGINE
	//Todo:Not compatible set Yaw yet
	HomeReset(HomeResetOrientation,0);
#endif
}
...
#ifdef PICO_CUSTOM_ENGINE
	//PICO AppSpaceWarp
	EHMDTrackingOrigin::Type lastOriginASW = GetTrackingOrigin();
	if (IsInGameThread()&&NextGameFrameToRender_GameThread.IsValid())
	{
			if(lastOriginASW != NewOrigin)
			NextGameFrameToRender_GameThread->BaseOffset = FVector::ZeroVector;
	};
#endif
...
#ifdef PICO_CUSTOM_ENGINE
	Result->BaseOrientation = FQuat::Identity;
	Result->BaseOffset = FVector::ZeroVector;
#endif
	return Result;
}
...
void FPICOXRHMD::UpdateStereoRenderingParams()
{
	...
	// Update IsUsingMobileMultiview and PixelDensity from Console Variables
	...
	*EyeLayerFound = MakeShareable(EyeLayer);
	...
	// Update Render Target and Render Viewport Sizes
	CalculateRenderTargetSize(GameSettings->RenderTargetSize);
	CalculateRenderViewportSize(GameSettings->EyeRenderViewport[0], GameSettings->EyeRenderViewport[1]);
...
#ifdef PICO_CUSTOM_ENGINE
//PICO AppSpaceWarp
void FPICOXRHMD::HomeReset(FHomeResetTypes ResetType,float Yaw)
{
	PXR_LOGI(PxrUnreal, "AppSpaceWarp HomeReset");
	if (CVarPICOEnableSpaceWarpInternal.GetValueOnAnyThread()
		&&NextGameFrameToRender_GameThread.IsValid()
		&&LastGameFrameToRender_GameThread.IsValid())
	{
		...
		if (ResetType & HomeResetPosition)
		{
			NextGameFrameToRender_GameThread->BaseOffset = LastGameFrameToRender_GameThread->Position;
			if (floorLevel)
				NextGameFrameToRender_GameThread->BaseOffset.Z = 0;
		}

		if (ResetType & HomeResetOrientation)
		{
			NextGameFrameToRender_GameThread->BaseOrientation=FRotator(0, FRotator(LastGameFrameToRender_GameThread->Orientation).Yaw - Yaw, 0).Quaternion();
		}
	}
}
void FPICOXRHMD::OnHMDRecentered()
{
	bAppNeedHomeReset=true;
}
#endif
...
#ifdef PICO_CUSTOM_ENGINE
			//PICO AppSpaceWarp
			EventManager->NativeLongHomePressedDelegate.Broadcast();
#endif
			break;
		}
...
bool FPICOXRHMD::InitializeSession()
{
	...
	{
		//SetAppEngineInfo2
		FString UnrealSDKVersion = "UE4_2.3.0";
...
void FPICOXRHMD::LoadFromSettings()
{
	...
	/*
	GameSettings->bAdaptiveResolutionMethodAutomatic = HMDSettings->AdaptiveResolutionMethod == EPICOXRAdaptiveResolutionMethod::Automatic;
...
void FPICOXRHMD::PostRenderBasePass_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneView& InView)
{
	if (RHIString != TEXT("Vulkan") ||
		...
		!CVarPICOEnableEyeTrackingFoveationRendering.GetValueOnRenderThread())
	{
		if (RenderBridge.IsValid())
		{
...
void FPICOXRHMD::CalculateRenderTargetSize(const class FViewport& Viewport, uint32& InOutSizeX, uint32& InOutSizeY)
{
	...
	CalculateRenderTargetSize(RenderTargetSize);
}

void FPICOXRHMD::CalculateRenderTargetSize(FIntPoint& RenderTargetSize)
{
	const FIntPoint MaxRenderTargetSize = GetMaxRenderTargetSize();

	// First, start with the ideal render target size for this device
	RenderTargetSize = GetIdealRenderTargetSize();

	// If PixelDensity is Adaptive,
	// Then allocate enough space for maximum PixelDensity.
	if (GameSettings->Flags.bPixelDensityAdaptive)
	{
		RenderTargetSize.X = FMath::RoundToInt(RenderTargetSize.X * GameSettings->PixelDensityMax);
		RenderTargetSize.Y = FMath::RoundToInt(RenderTargetSize.Y * GameSettings->PixelDensityMax);
	}

	// Otherwise, resize render target based on PixelDensity
	else
	{
		RenderTargetSize.X = FMath::RoundToInt(RenderTargetSize.X * GameSettings->PixelDensity);
		RenderTargetSize.Y = FMath::RoundToInt(RenderTargetSize.Y * GameSettings->PixelDensity);
	}

	// Align render target size to rectangles of a certain size.
	// Currently, we align to rectangles of 16x16 pixels.
	// This fixes issues with buffer sizing.
	const FIntPoint AlignmentRect(16, 16);
	const FIntPoint AlignmentMask(~(AlignmentRect.X - 1), ~(AlignmentRect.Y - 1));
	PXR_LOGV(PxrUnreal, "AlignmentRect:(%d,%d)", AlignmentRect.X, AlignmentRect.Y);

	RenderTargetSize.X = (RenderTargetSize.X + AlignmentRect.X - 1) & AlignmentMask.X;
	RenderTargetSize.Y = (RenderTargetSize.Y + AlignmentRect.Y - 1) & AlignmentMask.Y;

	// If using double wide (i.e. not using mobile multiview),
	// Then allocate enough space for both eyes
	if (!bIsUsingMobileMultiView)
	{
		RenderTargetSize.X *= 2;
	}

	// Shrink RenderTargetSize if too big
	if (RenderTargetSize.X > MaxRenderTargetSize.X || RenderTargetSize.Y > MaxRenderTargetSize.Y)
	{
		...
		RenderTargetSize = MaxRenderTargetSize;
	}

	PXR_LOGV(PxrUnreal, "Updated RenderTargetSize:(%d,%d)", RenderTargetSize.X, RenderTargetSize.Y);
}

void FPICOXRHMD::CalculateRenderViewportSize(FIntRect& LeftViewport, FIntRect& RightViewport)
{
	// Initialize the render viewport size from the render target size
	const FIntPoint RenderTargetSize = GameSettings->RenderTargetSize;
	FIntPoint ViewportSize = RenderTargetSize;
	...
	// If using Pixel Density is Adaptive,
	// then scale the Render Viewport based on the Pixel Density
	if (GameSettings->Flags.bPixelDensityAdaptive)
	{
		const float ResizeRatio = GameSettings->PixelDensity / GameSettings->PixelDensityMax;

		ViewportSize.X = FMath::RoundToInt(ViewportSize.X * ResizeRatio);
		ViewportSize.Y = FMath::RoundToInt(ViewportSize.Y * ResizeRatio);
		...
		ViewportSize.X = (ViewportSize.X + AlignmentRect.X - 1) & AlignmentMask.X;
		ViewportSize.Y = (ViewportSize.Y + AlignmentRect.Y - 1) & AlignmentMask.Y;

		// Ensure that ViewportSize is not larger than RenderTargetSize
		if (!bIsUsingMobileMultiView)
		{
			ViewportSize.X *= 2;
		}

		ViewportSize = ViewportSize.ComponentMin(RenderTargetSize);
	...
	// Set viewportSize
	LeftViewport = FIntRect(0, 0, ViewportSize.X, ViewportSize.Y);

	if (!bIsUsingMobileMultiView)
	{
		RightViewport = FIntRect(ViewportSize.X, 0, ViewportSize.X, ViewportSize.Y);
	}
	else // bIsUsingMobileMultiView
	{
		RightViewport = LeftViewport;
	}

	PXR_LOGV(PxrUnreal, "Updated ViewportSize:(%d,%d)", ViewportSize.X, ViewportSize.Y);
}
...
#ifdef PICO_CUSTOM_ENGINE
	EventManager->NativeLongHomePressedDelegate.AddRaw(this, &FPICOXRHMD::OnHMDRecentered);
#endif
