...
bool IPXR_BaseProvider::StartSenseDataProviderComplete(const FPICOSpatialHandle& FutureHandle, FPICOSenseDataProviderStartCompletion& Completion)
{
	...
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().StartSenseDataProviderComplete(FutureHandle.GetValue(), &SenseDataProviderStartCompletionBD));
...
bool IPXR_BaseProvider::QuerySenseDataComplete(const FPICOSpatialHandle& FutureHandle, FPICOSenseDataQueryCompletion& Completion)
{
	...
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().QuerySenseDataComplete(ProviderHandle.Value,FutureHandle.Value,&cPxrSenseDataQueryCompletionBD));
	Completion.SnapShotHandle = cPxrSenseDataQueryCompletionBD.snapshotHandle;
	PXR_LOGV(PxrMR, "Completion.QueryHandle:%llu", Completion.SnapShotHandle.Value);
	...
	if (Completion.FutureResult == EPICOResult::PXR_Error_LimitReached)
	{
		PXR_LOGE(PxrMR, "PXR_Error_LimitReached FutureHandle:%llu", FutureHandle.Value);
	}
	if (Completion.FutureResult != EPICOResult::PXR_Success)
	{
		return false;
	}
...
bool IPXR_BaseProvider::GetQueriedSenseData(const FPICOSpatialHandle& QueryResultHandle, FPICOQueriedSenseData& QueriedSenseData)
{
	...
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().GetQueriedSenseData(ProviderHandle.Value,&cPxrQueriedSenseDataGetInfoBD,&cPxrQueriedSenseDataBD));
	if (bResult)
	{
...
bool IPXR_BaseProvider::DestroySenseDataQueryResult(const FPICOSpatialHandle& QueryResultHandle)
{
	FRWScopeLock Lock(DestroyLock, SLT_Write);
	PXR_LOGV(PxrMR, "DestroySenseDataQueryResult:%llu", QueryResultHandle.Value);
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().DestroySenseDataQueryResult(QueryResultHandle.GetValue()));
	return bResult;
}
...
bool PXR_AnchorProvider::CreateSpatialAnchorAsync(const FPICOPollFutureDelegate& Delegate, const FTransform& InAnchorEntityTransform,EPICOResult& Result)
{
	FPICOSpatialHandle Handle;
	...
	int pxrResult=FPICOXRHMDModule::GetPluginWrapper().CreateSpatialAnchorAsync(ProviderHandle.Value,&cSpatialAnchorCreateInfoBD,&Handle.Value);
	bool bResult = PXRP_SUCCESS(pxrResult);
	Result=FPICOProviderManager::CastToPICOResult(static_cast<PxrResult>(pxrResult));
...
bool PXR_AnchorProvider::PersistSpatialAnchorAsync(AActor* BoundActor, EPICOPersistLocation PersistLocation, const FPICOPollFutureDelegate& Delegate,EPICOResult& Result)
{
	...
	int pxrResult=FPICOXRHMDModule::GetPluginWrapper().PersistSpatialAnchorAsync(ProviderHandle.Value,&SpatialAnchorPersistInfoBD,&Handle.Value);
	bool bResult = PXRP_SUCCESS(pxrResult);
	Result=FPICOProviderManager::CastToPICOResult(static_cast<PxrResult>(pxrResult));
...
bool PXR_AnchorProvider::UnpersistSpatialAnchorAsync(AActor* BoundActor, const FPICOPollFutureDelegate& Delegate,EPICOResult& Result)
{
	if (!IsAnchorValid(BoundActor))
	{
	...
	int pxrResult=FPICOXRHMDModule::GetPluginWrapper().UnpersistSpatialAnchorAsync(ProviderHandle.Value,&SpatialAnchorUnpersistInfoBD,&Handle.Value);
	bool bResult = PXRP_SUCCESS(pxrResult);
	Result=FPICOProviderManager::CastToPICOResult(static_cast<PxrResult>(pxrResult));
	if (bResult)
	{
...
bool PXR_AnchorProvider::ShareSpatialAnchorAsync(AActor* BoundActor, const FPICOPollFutureDelegate& Delegate,EPICOResult& Result)
{
	if (!IsAnchorValid(BoundActor))
	{
...
bool PXR_AnchorProvider::DownloadSharedSpatialAnchorsAsync(const FPICOAnchorLoadInfo& LoadInfo, const FPICOPollFutureDelegate& Delegate, TSet<FPICOSpatialHandle>& HandleSet,EPICOResult& Result)
{
	FPICOSpatialHandle Handle;
	bool bResult = false;
...
bool PXR_AnchorProvider::CreateSpatialAnchorComplete(const FPICOSpatialHandle& FutureHandle, FPICOSpatialAnchorCreateCompletion& completion)
{
	...
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().CreateSpatialAnchorComplete(ProviderHandle.Value,FutureHandle.GetValue(), &SpatialAnchorCreateCompletionBD));
...
bool PXR_AnchorProvider::PersistSpatialAnchorComplete(const FPICOSpatialHandle& FutureHandle, FPICOSpatialAnchorPersistCompletion& completion)
{
	...
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().PersistSpatialAnchorComplete(ProviderHandle.Value,FutureHandle.GetValue(), &SpatialAnchorPersistCompletionBD));
...
bool PXR_AnchorProvider::UnpersistSpatialAnchorComplete(const FPICOSpatialHandle& FutureHandle, FPICOSpatialAnchorUnpersistCompletion& completion)
{
	...
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().UnpersistSpatialAnchorComplete(ProviderHandle.Value,FutureHandle.GetValue(), &SpatialAnchorUnpersistCompletionBD));
...
bool PXR_AnchorProvider::ShareSpatialAnchorComplete(const FPICOSpatialHandle& FutureHandle, FPICOSpatialAnchorShareCompletion& completion)
{
	...
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().ShareSpatialAnchorComplete(ProviderHandle,FutureHandle.GetValue(), &SpatialAnchorShareCompletion));
	PXR_LOGV(PxrMR, "ShareSpatialAnchorComplete:%d", SpatialAnchorShareCompletion.futureResult);
	PXR_LOGV(PxrMR, "ShareSpatialAnchorComplete bResult:%d", bResult);

	completion.FutureResult = FPICOProviderManager::CastToPICOResult(SpatialAnchorShareCompletion.futureResult);
	return bResult && completion.FutureResult == EPICOResult::PXR_Success;
}

bool PXR_AnchorProvider::DownloadSharedSpatialAnchorsComplete(const FPICOSpatialHandle& FutureHandle, FPICOSharedSpatialAnchorDownloadCompletion& completion)
{
	...
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().DownloadSharedSpatialAnchorComplete(ProviderHandle,FutureHandle.GetValue(), &SharedSpatialAnchorDownloadCompletion));
	PXR_LOGV(PxrMR, "DownloadSharedSpatialAnchorsComplete:%d", SharedSpatialAnchorDownloadCompletion.futureResult);
	PXR_LOGV(PxrMR, "DownloadSharedSpatialAnchorsComplete bResult:%d", bResult);

	completion.FutureResult = FPICOProviderManager::CastToPICOResult(SharedSpatialAnchorDownloadCompletion.futureResult);
	return bResult && completion.FutureResult == EPICOResult::PXR_Success;
}

bool PXR_AnchorProvider::LoadAnchorEntityAsync(const FPICOAnchorLoadInfo& LoadInfo, const FPICOPollFutureDelegate& Delegate,EPICOResult& Result)
{
	FPICOSpatialHandle Handle;
...
bool PXR_AnchorProvider::GetAnchorLoadResults(const FPICOSpatialHandle& FutureHandle, TArray<FAnchorLoadResult>& LoadResult, EPICOResult& Result)
{
	FPICOSenseDataQueryCompletion SenseDataQueryCompletion;

	if (QuerySenseDataComplete(FutureHandle, SenseDataQueryCompletion))
	{
		Result = SenseDataQueryCompletion.FutureResult;
		PXR_LOGV(PxrMR, "SenseDataQueryCompletion.FutureResult:%d",Result);

		if (PXR_SUCCESS(SenseDataQueryCompletion.FutureResult))
		{
			FPICOQueriedSenseData QueriedSenseData;
			if (GetQueriedSenseData(SenseDataQueryCompletion.SnapShotHandle, QueriedSenseData))
			{
				bool bResult = false;
				for (auto Entity : QueriedSenseData.QueriedSpatialEntityInfos)
				{
					...
					bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().RetrieveSpatialEntityAnchor(SenseDataQueryCompletion.SnapShotHandle,&cPxrSpatialEntityAnchorRetrieveInfo,&cAnchorLoadResult.AnchorHandle.Value));
				...
				bool Destroy = DestroySenseDataQueryResult(SenseDataQueryCompletion.SnapShotHandle);

				if (!Destroy)
				{
					PXR_LOGV(PxrMR, "DestroySenseDataQueryResult failed!!!");
				}

				return bResult;
			}
			
			DestroySenseDataQueryResult(SenseDataQueryCompletion.SnapShotHandle);
			PXR_LOGV(PxrMR, "PXR_AnchorProvider->GetQueriedSenseData failed!!!");
		}
	}
...
bool PXR_AnchorProvider::DestroyAnchorByHandle(const FPICOSpatialHandle& AnchorHandle)
{
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().DestroyAnchor(AnchorHandle));
	return bResult;
}
...
bool PXR_AnchorProvider::GetAnchorEntityUUID(AActor* BoundActor, FPICOSpatialUUID& OutAnchorUUID)
{
	if (!IsAnchorValid(BoundActor))
	{
		return false;
	...
	FPICOSpatialHandle AnchorHandle = AnchorComponent->GetAnchorHandle();
	PxrUUid PxrAnchorUUID;
	bool bResult = PXRP_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().GetAnchorUuid(AnchorHandle.GetValue(), &PxrAnchorUUID));

	PXR_LOGV(PxrMR, "GetAnchorEntityUUID Call PxrAPI Result[%d], Handle[%llu]", bResult, AnchorHandle.GetValue());
...
bool PXR_AnchorProvider::GetAnchorPose(UPICOAnchorComponent* AnchorComponent, FTransform& OutAnchorPose)
{
	if (!IsAnchorValid(AnchorComponent))
	{
		return false;
	...
	bool bResult = PXR_SUCCESS(FPICOXRHMDModule::GetPluginWrapper().LocateAnchor(&cPxrAnchorLocateInfoBD, &cPxrSpaceLocation));
...
bool PXR_AnchorProvider::UpdateAnchor(UPICOAnchorComponent* AnchorComponent)
{
...
bool PXR_MeshProvider::GetSpatialTriangleMeshInfos(const FPICOSpatialHandle& FutureHandle,const FTransform& TrackingToWorld,
                                                   ...
                                                   TArray<FPICOSpatialMeshInfo>& MeshInfos, EPICOResult& Result)
{
	...
	bool bDestroyResult = DestroySenseDataQueryResult(SenseDataQueryCompletion.SnapShotHandle);
	PXR_LOGV(PxrMR, "DestroySenseDataQueryResult:%d", bDestroyResult);
	PXR_LOGV(PxrMR, "GetSpatialTriangleMeshInfos Result:%d", bResult);

	bResult = bResult && bDestroyResult;
...
void PXR_MeshProvider::SetLastUUIDToMRMeshInfoMap(const TMap<FPICOSpatialUUID, FPICOSpatialMeshInfo>& UUIDToMRMeshInfoMap)
{
	FScopeLock Lock(&CriticalSection);
...
bool PXR_MeshProvider::RequestSpatialTriangleMesh(const FPICOPollFutureDelegate& Delegate,EPICOResult& Result)
{
	FPICOSpatialHandle Handle;
	PxrSenseDataQueryInfo cSenseDataQueryInfoBD = {};
	cSenseDataQueryInfoBD.type = PxrStructureType::PXR_TYPE_SENSE_DATA_QUERY_INFO;
...
bool PXR_SceneProvider::GetSpatialSceneInfos(const FPICOSpatialHandle& FutureHandle, TArray<FPICOMRSceneInfo>& SceneInfos, EPICOResult& Result)
{
	FPICOSenseDataQueryCompletion SenseDataQueryCompletion;
	bool bResult = false;
	if (QuerySenseDataComplete(FutureHandle, SenseDataQueryCompletion))
	{
		...
		if (PXR_SUCCESS(SenseDataQueryCompletion.FutureResult))
		{
			...
			if (GetQueriedSenseData(SenseDataQueryCompletion.SnapShotHandle, QueriedSenseData))
			{
				//Todo:It may not be appropriate to put it here
				...
				bResult = DestroySenseDataQueryResult(SenseDataQueryCompletion.SnapShotHandle);

				if (!bResult)
				{
					PXR_LOGE(PxrMR, "DestroySenseDataQueryResult failed!!!");
				}
			}
			else
			{
				PXR_LOGV(PxrMR, "PXR_SceneProvider->GetQueriedSenseData failed!!!");
				Result = EPICOResult::PXR_Error_Unknow;
			}
		}
	}
